...
Phase 2 では `notify` のイベントを相対 path の dirty set へ集約し、500ms の debounce と1秒の batch window の後に変更ファイルだけを解析します。変更ファイルごとに `BEGIN IMMEDIATE` transaction を開始し、正規化データと FTS5 を同時に置換します。

`file_states` は `fresh`、`stale`、`missing` を持ちます。構文診断または読み込み失敗時には直前の正常データを保持し、対象を `stale` として検索から除外します。rename は旧 path の削除と新 path の追加として扱い、delete では古い file・symbol・chunk を削除します。
