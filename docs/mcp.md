...
リポジトリは事前に CLI で登録します。

```bash
astral register my-repo /path/to/repository
```

すべての tool input は `repository_name` を必須とし、登録済み name から canonical root と専用 index DB を解決します。MCP request から repository root path を直接受け付けません。
...
{
  "repository_name": "my-repo",
  "symbolId": "symbol:my-repo:src/auth/session.ts:createSession:12:0"
}
```
...
{
  "repository_name": "my-repo",
  "symbol": "symbol:my-repo:src/auth/session.ts:createSession:12:0",
  "limit": 100
}
```
...
`astral serve` は `rmcp` の stdio transport で read-only tools を公開します。`read_code` は最大120行・32,000 bytes、検索・relationship toolは最大100件に制限し、切り詰め時は `truncated: true` を返します。未登録 repository name、登録 root 外のpath、編集、任意コマンド実行は拒否または非公開です。レスポンスには `repositoryName` を含めます。
