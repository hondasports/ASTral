...
```bash
cargo run -- register my-repo .
cargo run -- unregister my-repo
cargo run -- index my-repo
cargo run -- search-code my-repo "RepositoryRoot"
cargo run -- find-symbol my-repo "RepositoryRoot"
cargo run -- read-symbol my-repo "symbol:my-repo:src/repository.rs:RepositoryRoot:..."
cargo run -- status my-repo
cargo run -- evaluate my-repo
```
...
GitHub Actions の `CI` workflow は、push・Pull Request・手動実行に対して Ubuntu、Windows、macOS（Apple Silicon）の各runnerで上記4コマンドを実行し、最後に `astral --help` のCLI smoke testを実行します。Cargo registry、Git依存、`target`はOS別にキャッシュされます。
...
別の対象リポジトリを指定する場合は、`register` で名前と repository root を登録してから、その名前を各コマンドへ渡してください。
...
```sh
ASTRAL_DATA_DIR=".astral-data" cargo run -- evaluate my-repo
```
